...
export default function EditTransactionForm({ transaksi }: { transaksi: any }) {
  ...
  const [formData, setFormData] = useState({
    id_produk: transaksi.id_produk.toString(),
    id_pelanggan: transaksi.id_pelanggan.toString(),
    tanggal_transaksi: new Date(transaksi.tanggal_transaksi).toISOString().split('T')[0],
    total_harga: transaksi.total_harga.toString(),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/transactions/${transaksi.id_transaksi}`, {
        method: 'PUT',
